...
async function requireOwnedJob(userId, jobId) {
  const id = String(jobId || "").trim();
  if (!id) throw { code: 40003, message: "缺少 AI 任务 ID" };
  const result = await db.collection("ai_jobs").where({ _id: id, userId }).limit(1).get();
  if (!result.data || result.data.length === 0) throw { code: 40401, message: "AI 任务不存在或无权访问" };
