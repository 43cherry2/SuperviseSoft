...
exports.main = async (event, context) => {
  ...
  try {
    ...
    const token = await callAuthApi(context, event, "/auth/v1/signup", "POST", {
      phone_number: phone,
    ...
    await writeLoginLog(authUid, phoneMasked, "register", true, "");
    ...
    await safeWriteLoginLog(authUid, phoneMasked, "register", false, error.message);
    return fail(error.code || 40001, error.message || "注册失败");
  }
};
...
async function safeWriteLoginLog(authUid, phoneMasked, loginType, success, errorMessage) {
  try {
    ...
    // Avoid hiding the real auth error with a secondary logging failure.
  }
}
